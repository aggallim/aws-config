        ...
        if rule_parameters['WhiteListedVPC']:
            if vpc_id in rule_parameters['WhiteListedVPC']:
                evaluations.append(build_evaluation(
                    vpc_id, 'COMPLIANT', event, annotation='This is a WhiteListed VPC.'))
                continue

            evaluations.append(build_evaluation(
                vpc_id, 'NON_COMPLIANT', event, annotation='VPC is Default'))
...
def evaluate_parameters(rule_parameters):
    ...
    for key in rule_parameters:
        if key not in ALLOWED_PARAMETER_NAMES:
            raise ValueError('The parameter ' + key +
    ...
    if 'WhiteListedVPC' in rule_parameters:
        whitelisted_vpcs = rule_parameters['WhiteListedVPC'].replace(
            ' ', '').split(',')
        for vpc in whitelisted_vpcs:
            if not vpc.startswith('vpc-'):
                raise ValueError(
                    'The parameter "WhiteListedVPC" is not a valid vpc-id format.')
